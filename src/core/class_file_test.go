...
func TestItShouldAllocateAConstantPoolFromTheBytecodeSize(t *testing.T) {
	b := []byte{0xCA, 0xFE, 0xBA, 0xBE, 0x00, 0x00, 0x00, 0x34, 0x00, 0x02}
	...
	if len(cf.ConstantPool) != 1 {
		t.Errorf("Expected constant pool of size 1, got %d", len(cf.ConstantPool))
	}
}
